import { TurnMessageSender, PlayersMessageSender } from "../network/message.ts";
...
export class Turn {
    ...
    public start() {
        this.board.players.forEach(player => this.turnMessageSender.sendMessage(player))
    }

    private end() {
        this.currentPlayerPosition = ++this.currentPlayerPosition % Ruler.ACTIVE_PLAYER_NUMBER
        this.moveCount = 0
    ...
    private checkLooseCondition() : boolean {
        ...
        const looser = this.board.getActivePlayers().find(player => looseCondition.hasLost(this.board, player))
        ...
        looser.hasLost = true
        this.board.players.forEach(player => this.playersMessageSender.sendMessage(player))
